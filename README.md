...
- Windows版「peercast_encoder_for_pecaone」とのQRペアリング、Tailscale経由のHLS視聴、画質切替。

### Windows経由で視聴する

スマホとWindowsの両方でTailscaleに接続し、Windowsアプリで「ペアリングQRを作成」を押します。スマホの「設定」から「Windowsとペアリング」を開き、QRを読み取ってください。続けて「視聴方法」を「Windows経由」に変更します。画質は「自動」が初期値で、視聴中にも変更できます。

Windows経由は携帯回線でも利用できます。番組のリレーと変換はWindows側で行われ、スマホ側の待受ポートは使用しません。Windows経由を選んだ場合、既定のSPチャンネル一覧はWindowsアプリから取得します。SPの接続元・ポート判定もWindows側になるため、WindowsのPeerCastStationで外部リレーポートを開放してください。SPで既定以外の使用ポートを設定している場合は、Windows PCのブラウザからSPの設定を確認してください。設定の「SPでポート開放を確認」では、Windows側からの一覧取得件数とPeerCastStationのリレー待受状態を確認できます。Windowsアプリがこの機能に未対応の場合は更新が必要で、スマホからのSP直接取得へは自動的に切り替えません。p@など他のYPは引き続きスマホから取得します。直接視聴に戻す場合は「視聴方法」を「端末で直接」に変更してください。登録解除や再ペアリング後に認証が無効になった場合は、QRを再度読み取ってください。
